...
$access_config = json_decode('{"get_table":{"allow_action":[2,1],"deny_action":[0],"field_disallow_view":{"1":[],"2":[]},"field_disallow_update":{"1":["lcc","uname","ufam","uoth","balance","email","phone","password","access","salt","id"],"2":["lcc","id"]},"record_allow_update":[2],"record_allow_view":{"0":2,"OWNER":["id"]}},"get_it":{"allow_action":[2,1],"deny_action":[0],"field_disallow_view":{"1":["id"],"2":[]},"field_disallow_update":{"1":["lcc","uname","ufam","uoth","balance","email","phone","password","access","salt","id"],"2":["lcc","id"]},"record_allow_update":[2],"record_allow_view":{"0":2,"OWNER":["id"]}},"update":{"allow_action":[2,1],"deny_action":[0],"field_disallow_view":{"1":["id"],"2":[]},"field_disallow_update":{"1":["lcc","uname","ufam","uoth","balance","email","phone","password","access","salt","id"],"2":["lcc","id"]},"record_allow_update":[2],"record_allow_view":{"0":2,"OWNER":["id"]}},"modal_editor":{"allow_action":[2,1],"deny_action":[0],"field_disallow_view":{"1":[],"2":[]},"field_disallow_update":{"1":["lcc","uname","ufam","uoth","balance","email","phone","password","access","salt","id"],"2":["id","lcc"]}}}', true);
...
class users {
    ...
    function doreg(){
        $users = $this->db->select('users', array('id'=>$_SESSION["user_id"]));
        //var_dump($users);
        if($users[0]['email'] == ''){
           $r=  $this->db->update('users', array('email'=>$_POST['email']), array('id'=>$_SESSION["user_id"]));
            //print 'upd'; var_dump($r);
        }
        include ('tpl/goreg.html');
        }
    function update() {
        global $access_config;
    ...
    function get_table() {
        ...
        $page_settings = array(
            ...
            "selectSQL" => $this
                ->db
                ->pre_select('users', $where, $get_fields) ,
    ...
    function get_list() {
        ...
        if ($allowed) $users = $this
            ->db
            ->select('users', $where, $get_fields);
