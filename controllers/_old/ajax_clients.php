...
$page_settings = array(
	"selectCountSQL" => "select count(*) as totalrows from `clients` where not `trash`",    // CONFIGURE

	"selectSQL" => "select `client_id`, 
	`title`,  
	`director`, 
	`email`, 
	`tel`
	from `clients` where not `trash`",  // CONFIGURE
...
foreach($data['page_data'] as $key => $row) {
    ...
    $row['actions'] = "<button class=\"btn btn-primary btn-xs\" data-toggle=\"modal\" data-target=\"#myModal\" data-whatever=\"{$row['client_id']}\"><i class=\"fa fa-edit\"></i></button>
    <button class=\"btn btn-primary btn-xs\" data-toggle=\"modal\" data-target=\"#EmailModal\" data-whatever=\"{$row['client_id']}\"><i class=\"fa fa-envelope\" ></i></button>
	<button class=\"btn btn-primary btn-xs trash delclient\" data-whatever=\"{$row['client_id']}\"><i class=\"fa fa-trash\" ></i></button>";
