$('#category').on('change', function() {
	...
	dataString = 'action=get-services&type='+selected+'&soc='+$("input[name=soc]").val();
	$.ajax({
		type: "POST",
		url: "/services/get_list/",
		data: dataString,
		cache: false,
...
function selectService(ServiceID) {
	...
	$.ajax({
		...
		success: function(data){
			if(data) {
				...
				$form.find('.form-group[data-name=quantity]').find('input[name=quantity]').attr('value','');
				$form.find('.form-group[data-name=quantity]').show();
				$("#additional").html('');
				$("#order_quantity").prop("readonly", false);
			}
	...
	var autoModeAllowedForServices = [3,4,7,11,12,14,30,31,32,33,36,37,39,40,41,43,44,48,49,52,64,66,71,72,73,91,92,94,95];
...
$("#show-primer").click(function() {
	if($("#primer").is(':visible')) {
		$("#show-primer").html('Пример распространения.');
		$("#primer").hide( "slow" );
	} else {
		$("#show-primer").html('Скрыть.');
		$("#primer").show( "slow" );
	}
});