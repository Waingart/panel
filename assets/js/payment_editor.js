...
$('#myModal').on('show.bs.modal', function (event) { // Если открыли окно редактирования (создания) счета...
  ...
  if(recipient == 'new'){ // По кнопке узнаем, что это создание новго счета
    ...
    $.getJSON('/panel/invoices-filldata/', function(data){  // Берем данные с сервера (список клиентов и услуг)
      clear_doc();
    ...
    if (recipient != undefined){
      $.getJSON('/panel/invoices-filldata/'+recipient, function(data){
...
function load_docs_list(){
	$("#payments").bs_grid({
        ...
        ajaxFetchDataURL: "/panel/payments-ajax",
