...
function show_noticle(){
  ...
  var link    = $('<a />', { href: '#', 'data-toggle':"modal", 
    'data-target':"#myModal",
    'data-backdrop':"static"})
    .html('Внимание! Есть неоплаченные счета. Оплатите онлайн прямо сейчас.')
    .css(link_css)
    .hover(function () {
  ...
  var btn     = $('<a />', {
    ...
    'data-whatever':"all",
    href   : '#'
  }).html('Оплатить онлайн').css(btn_css)
  var close   = $('<a />', {
    'class'         : 'pull-right',
    href            : '#',
    'data-toggle'   : 'tooltip',
...
////#myInput - id элемента, которому необходимо установить фокус
 //// $('#myInput').focus();
  $('#myModal iframe').css('min-height', '700px');
 var button = $(event.relatedTarget);
 if(button.data('whatever') == 'all'){
    loadIframe('payform', '/payments/go_count/');
     //loadIframe('payform', '/payments/go_pay/');
 }else{
     //loadIframe('payform', '/payments/go_pay_one/'+button.data('whatever')+'/');
     loadIframe('payform', '/payments/go_count/'+button.data('whatever')+'/');
 }
 
  //alert( button.data('whatever')) ;
});
function formsucc(){
    $('#myModal iframe').css('min-height', 'auto');
}
function closeform(){
    $('#myModal').modal('hide');
}